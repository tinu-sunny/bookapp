...
const handleUpdate =async ()=>{
        ...
        <div style={{ width: '100%' }}>
          ...
          <div className='mt-10 flex justify-between gap-4 mb-10 ' style={{ width: '100%' }}>
            ...
            <div className='flex flex-col justify-center' style={{ width: '40%' }}>
             {!edit? <p className='leading-relaxed text-1xl italic' onClick={()=>setEdit(true)}>{adminData.bio}</p>:<Textarea   onChange={(e) =>
             setAdminData({
  ...adminData,
  bio: e.target.value
})
                  } placeholder={adminData.bio} value={adminData.bio}></Textarea> }
            </div>
            {/* edit page */}
            <Card className='text-center p-5' style={{ width: '50%' }}>
              ...
              <div className='text-center'>
                <div className='flex justify-center flex-col'>
                 <label htmlFor='image'>
                  <TextInput id='image' type='file' hidden  onChange={(e) => handleimageupload(e)}></TextInput>
                    <img src={preview && preview.length > 0 ?preview :`${serverURL}/uploads/${adminData.profile}`} id='image' alt="" className='w-40 h-40 rounded-full object-cover'   />
                    <div id='image' className='mt-[-25px] ml-[100px] w-4 '><MdEdit size={"30px"} /></div>
                 </label>
                </div>
                {/* edit box */}
                <div className='mt-10' >
                     {!edit?   <p className='mb-4 border-2 border-gray-300 rounded p-1  bg-gray-100' onClick={()=>setEdit(true)} placeholder='name' >{adminData.username}</p>:<TextInput className='mb-4' value={adminData.username} placeholder={adminData.username}   onChange={(e) =>
           setAdminData({
  ...adminData,
  username: e.target.value
               ...
               {!newpass ?<div>
                    <TextInput className='mb-4' placeholder='Current password' onChange={(e)=>{setOldpassword(e.target.value)}}></TextInput>
  ...
  password: e.target.value
              ...
              <div className='flex justify-center gap-5 w-full'>
                ...
                <Button className='w-full bg-green-500' onClick={handleUpdate}>Update</Button>
              </div>
            </Card>
          </div>
        </div>
